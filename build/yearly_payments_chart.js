...
var Bar = React.createClass({
  ...
  render: function () {
    return React.createElement("rect", { fill: this.props.color,
...
var DataSeries = React.createClass({
  ...
  render: function () {
    ...
    var yScale = d3.scale.linear().domain([0, d3.max(this.props.data)]).range([0, this.props.height]);
    ...
    var bars = this.props.data.map(function (point, i) {
      return React.createElement(Bar, { height: yScale(point), width: xScale.rangeBand(),
...
var YearlyPaymentsChart = React.createClass({
  displayName: "YearlyPaymentsChart",
...
var StackedBarChart = React.createClass({
  ...
  render: function () {
    ...
    var yScale = d3.scale.linear().domain([0, d3.max(totals)]).range([0, this.props.height]);
